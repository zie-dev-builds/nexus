...
export function NexusDashboard() {
  ...
  const [messages, setMessages] = useState<string[]>([
    "Hey! Are we still on for the creative jam?",
    "Absolutely. I saved a spot for us ☕",
  ]);
        ...
        if (Array.isArray(data.posts)) {
          setPosts(
            data.posts.map((post: any) => ({
              ...
              avatar: post.author?.image ?? "https://i.pravatar.cc/100?img=68",
      ...
      setPosts((currentPosts) => [
        {
          ...
          image: created.post.imageUrl ?? undefined,
          likes: 0,
          comments: 0,
          liked: false,
        },
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("/api/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ receiverId: people[0].handle.replace("@", ""), body: message }),
      });
          ...
          <div className="mx-auto max-w-[760px] px-5 py-8 lg:px-10">
            ...
            <div className="card mb-7 p-5">
              ...
              <div className="mt-4 flex items-center justify-between border-t border-[var(--line)] pt-4">
                ...
                <button onClick={addPost} className="rounded-xl bg-[var(--accent)] px-5 py-2 text-sm font-bold text-white disabled:opacity-40" disabled={!composer.trim()}>
                  Post
                </button>
              </div>
            </div>
            ...
            <div className="space-y-5">
              {posts.length === 0 ? (
                <div className="card p-8 text-center text-sm opacity-60">Your feed is waiting. Share your first post.</div>
              ) : (
                posts.map((post) => (
